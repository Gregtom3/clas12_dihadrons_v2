...
void make_tree(const config::AnalysisConfig& cfg,const config::Dataset& data,const fs::path& output,size_t max_files,size_t max_events){
 ...
 clas12root::HipoChain chain;for(const auto&f:files)chain.Add(f);auto* setup=chain.GetC12Reader();const bool mc=data.kind==config::DatasetKind::monte_carlo;if(mc)setup->db()->turnOffQADB();setup->addAtLeastPid(11,1);setup->addAtLeastPid(211,1);setup->addAtLeastPid(-211,1);auto&reader=chain.C12ref();if(!mc)reader->db()->qadb_requireOkForAsymmetry(true);
...
void fit_tree(const fs::path&input,const fs::path&outdir,std::string dataset_id){
 ...
 nlohmann::json j={{"schema_version",1},{"dataset",dataset_id},{"channel","piplus_piminus"},{"bin_id","integrated"},{"background_method","none"},{"counts",{{"input_events",events.size()},{"selected_events",events.size()},{"dihadron_candidates",events.size()}}}};const auto&defs=beam_spin_modulations();for(size_t i=0;i<7;++i){j["amplitudes"][defs[i].id]={{"value",result.amplitudes[i]},{"uncertainty",result.errors[i]}};j["covariance"].push_back(result.covariance[i]);}j["fit"]={{"converged",result.converged},{"iterations",result.iterations},{"negative_log_likelihood",result.negative_log_likelihood},{"helicity_balanced",true}};std::ofstream(outdir/"piplus_piminus_integrated.json")<<j.dump(2)<<'\n';
