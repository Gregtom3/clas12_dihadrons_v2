...
class AnalysisGui final:public TGMainFrame {
 ...
 void launch_local(const std::vector<std::string>& args){const auto log=log_path();local_pid_=fork();if(local_pid_<0)throw std::runtime_error("fork failed");if(local_pid_==0){const int fd=open(log.c_str(),O_WRONLY|O_CREAT|O_TRUNC,0644);if(fd>=0){dup2(fd,STDOUT_FILENO);dup2(fd,STDERR_FILENO);close(fd);}std::vector<char*> av;for(const auto&a:args)av.push_back(const_cast<char*>(a.c_str()));av.push_back(nullptr);execvp(av[0],av.data());_exit(127);}status("Started local pipeline PID "+std::to_string(local_pid_)+".");}
